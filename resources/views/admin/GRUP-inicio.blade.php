...
@section('GRUP-inicio')
    <div
        ...
        <table class="table tabla-dorada w-100 fs-5  text-center">
            ...
            <tbody>
                @foreach ($alumnos as $grupo)
                    <tr class="">
                        <td class="texto-color-resalte border border-1 border-fondo">{{ $grupo['id'] }}</td>
                        <td class="texto-color-resalte border border-1 border-fondo">{{ $grupo['nombreGrupo'] }}</td>
                        <td class="texto-color-resalte border border-1 border-fondo">
                            <ul class="fs-5 container-fluid row row-cols-3">
        ...
        <a class="fs-5 enlace" href="{{ route('grupo.create') }}">Crear nuevo grupo</a>
    </div>
@endsection
